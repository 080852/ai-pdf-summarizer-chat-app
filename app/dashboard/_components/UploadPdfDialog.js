...
// // // 2 step for getting the fieldid page on the workspace.
// "use client";
...
//   const addFileEntry = useMutation(api.fileStorage.AddFileEntryToDb);
//   const getFileUrl = useMutation(api.fileStorage.getFileUrl);
//   const embeddDocument = useAction(api.myAction.ingest); // Ensure correct reference
//   const { user } = useUser();
//   const [file, setFile] = useState(null);
//   const [loading, setLoading] = useState(false);
//   const [fileName, setFileName] = useState("");
...
//       // Fetch text from PDF for embedding
//       const apiResponse = await axios.get(`/api/pdf-loader?pdfUrl=${encodeURIComponent(fileUrlResponse.fileUrl)}`);
